import {
  dp_background_setter,
  capitalizeFirstTwoCharacter,
} from '../display-picture.middleware.js';
...
const homeOuterScroll = () => {
  ...
  outerContainer.addEventListener(
    ...
    function (event) {
      ...
      const atBottom =
        innerContainer.scrollHeight - innerContainer.scrollTop ===
...
`;
  ...
  const getAllUserPost = () => {
    document.addEventListener('DOMContentLoaded', async () => {
      const response = await fetch(`${API_URL}/api/post/user`, {
        method: 'GET',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
        if (result.posts.length === 0) {
          const noPostHTML = ` <div class="post background-white rounded-3 shadow-sm p-2 mb-2 d-flex flex-column " style=" width: 100%;height: auto;" >
      <div class="dashed-box" style=" border: 2px dashed #ccc;
    border-radius: 0.5rem;
    padding: 2rem">
  <div class="post-top background-white d-flex justify-content-center align-items-center flex-column" style="height: 20rem;">
      ...
      document.addEventListener('click', async function (event) {
        ...
        if (editProfileButton) {
          ...
          if (response.ok) {
            ...
            const usernameInput = modalElement.querySelector(
              '#edit-User-username'
            );
    ...
    document.addEventListener('DOMContentLoaded', () => {
      document.addEventListener('click', async function (event) {
        ...
        if (updateProfileButton) {
          ...
          let response = await fetch(`${API_URL}/api/user/userDetail`, {
            method: 'POST',
            credentials: 'include',
            headers: {
              'Content-Type': 'application/json',
          ...
          if (!response.ok) {
            ...
            if (
              ...
              const errorBox = modalElement.querySelector(
                '.error-container-edit-user-modal'
              );
              if (errorBox) {
...
const friendPageSection = () => {
  //------------------load friends page
  document.addEventListener('DOMContentLoaded', async () => {
    const response = await fetch(`${API_URL}/api/user/getAllFriends`, {
      method: 'GET',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
  ...
  document.addEventListener('click', async function (event) {
    ...
    if (friendListButton) {
      ...
      if (response.ok) {
        ...
        const friendRequestList = new Set(
          result.currentUser.friend.requestReceived
        );
        const friends = result.users.filter((user) =>
          friendRequestList.has(user._id)
        );
...
const memebersPageSection = () => {
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    <div class=" ms-auto userTypeButton">
      ...
      if (user.friend.friendList.some((user) => user === result.userId)) {
        //checks user is in friendList
        ...
        userContainer
          .querySelector('.unFriendButton')
          .setAttribute('data-otherUserId', user._id);
      } else if (
        user.friend.requestReceived.some((user) => user === result.userId)
      ) {
        //checks user is in requestReceived
        ...
        userContainer
          .querySelector('.userTypeButton')
          .append(requestSentButton);
        userContainer
          .querySelector('.requestSentButton')
          .setAttribute('data-otherUserId', user._id);
      } else if (
        user.friend.requestSent.some((user) => user === result.userId)
      ) {
        //checks user is in requestSent
        ...
        userContainer
          .querySelector('.frndConfirmButton')
          .setAttribute('data-otherUserId', user._id);
        ...
        userContainer
          .querySelector('.frndDeleteButton')
          .setAttribute('data-otherUserId', user._id);
      } else {
...
<div class="comment-container"></div>
    ...
    if (friendContainer) {
      if (user._id === element.postObj.user._id) {
        ...
        user.friend.friendList.some((user) => user === element.postObj.user._id)
      ) {
        ...
        user.friend.requestSent.some(
          (user) => user === element.postObj.user._id
        )
      ) {
        ...
        user.friend.requestReceived.some(
          (user) => user === element.postObj.user._id
        )
      ) {
    ...
    if (commentPost) {
      ...
      commentPost.setAttribute(
        'data-comment-count',
        element.postObj.comment.length
      );
    }
    ...
    function checkOverflow() {
      ...
      let maxWidthStr = window
        .getComputedStyle(captionElement)
        .getPropertyValue('max-width');
      let maxHeightStr = window
        .getComputedStyle(captionElement)
      ...
      const isHorizontallyOverflowing =
        captionElement.scrollWidth > visibleWidth;
      const isVerticallyOverflowing =
        captionElement.scrollHeight > visibleHeight;
      ...
      if (isHorizontallyOverflowing || isVerticallyOverflowing) {
        ...
        postHTML
          .querySelector('.showmore-container')
          .insertAdjacentHTML('beforeend', showMore);
      }
    }
    ...
    const profilePicture = () => {
      if (element.postObj.user.profilepicture) {
        ...
        const firstTwoLetter = capitalizeFirstTwoCharacter(
          element.postObj.user.username
        );
...
const dropdownSection = () => {
  ...
  document.addEventListener('DOMContentLoaded', async function (event) {
    const response = await fetch(`${API_URL}/api/user/userCount`, {
      method: 'GET',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
  ...
  logoutAllButton.addEventListener('click', async (event) => {
    ...
    const response = await fetch(`${API_URL}/api/user/logoutAll`, {
      method: 'POST',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (response.ok) {
      ...
      if (result.profilepicture) {
        ...
        const removeButtonContainer = document.querySelector(
...
const postSection = () => {
  ...
  document.addEventListener('click', async function (event) {
    ...
    if (deletePost) {
      ...
      const response = await fetch(`${API_URL}/api/post/deletePost`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  document.addEventListener('click', async function (event) {
    ...
    if (editPostButton) {
      ...
      const response = await fetch(`${API_URL}/api/post/fetchCaption`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  document.addEventListener('click', async function (event) {
    ...
    if (editPostButton) {
      ...
      const response = await fetch(`${API_URL}/api/post/editPost`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  document.addEventListener('click', async function (event) {
    ...
    if (postLikeButton) {
      ...
      const response = await fetch(`${API_URL}/api/post/likePost`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  document
    .querySelectorAll('.editPostCancel, .editPostClose,.editUserCancel')
    .forEach((button) => {
...
const commentSection = () => {
  ...
  const deleteComment = async (
    ...
    commentDropDownHTML.addEventListener('click', async function (event) {
      ...
      if (deleteButton) {
        ...
        const commentCountNumber =
          postContainer.getAttribute('data-comment-count');
        ...
        const response = await fetch(`${API_URL}/api/comment?${queryParams.toString()}`, {
          method: 'DELETE',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
    ...
    commentHTML.addEventListener('click', async function (event) {
      ...
      if (commentLikeButton) {
        const commentLikeIcon =
          commentLikeButton.querySelector('.comment-like-icon');

        const response = await fetch(`${API_URL}/api/comment/like`, {
          method: 'POST',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
        ...
        if (response.ok) {
          ...
          let commentLikeCount = commentLikeButton.querySelector(
            '.comment-like-count'
          );
  ...
  const fetchCommentStructure = (comment, result, commentContainer, postId) => {
    ...
    const commentDelete = commentDropDownHTML.querySelector(
  ...
  const fetchComments = async (postId, commentContainer) => {
    ...
    if (response.ok) {
      ...
      commentContainer.addEventListener('click', async function (event) {
        const showmoreButton = event.target.closest(
          '.show-more-comment-toggleButton'
        );
        if (showmoreButton) {
          commentContainer.innerHTML = '';
  ...
  document.addEventListener('click', async function (event) {
    ...
    if (commentPostButton) {
      ...
      const commentContainer =
        postContainer.querySelector('.comment-container');
      commentContainer.innerHTML = '';
  ...
  const addFriend = () => {
    document.addEventListener('click', async function (event) {
      ...
      if (addFriendButton) {
        ...
        const response = await fetch(`${API_URL}/api/user/addFriend`, {
          method: 'POST',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
  ...
  const unFriend = () => {
    document.addEventListener('click', async function (event) {
      ...
      if (unFriendButton) {
        ...
        const response = await fetch(`${API_URL}/api/user/unFriend`, {
          method: 'POST',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
  ...
  const cancelFriendRequest = () => {
    document.addEventListener('click', async function (event) {
      ...
      if (cancelButton) {
        ...
        const response = await fetch(`${API_URL}/api/user/cancelFriendRequest`, {
          method: 'POST',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
  ...
  const confirmFriendRequest = () => {
    document.addEventListener('click', async function (event) {
      ...
      if (confirmButton) {
        ...
        const response = await fetch(`${API_URL}/api/user/confirmFriendRequest`, {
          method: 'POST',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
  ...
  const deleteFriendRequest = () => {
    document.addEventListener('click', async function (event) {
      ...
      if (deleteButton) {
        ...
        const response = await fetch(`${API_URL}/api/user/deleteFriendRequest`, {
          method: 'POST',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
