...
export const dp_background_setter = (name, container) => {
  //import to know
  switch (true) {
    case /^[A-Ea-e]$/.test(name.charAt(0)):
      ...
      document
        .querySelectorAll('#navbar-user-dp, #posting-dp')
        .forEach(function (el) {
    ...
    case /^[F-Jf-j]$/.test(name.charAt(0)):
      if (container) {
        container.style.background =
          'linear-gradient(135deg, #a2c0cc, #fceea7)';
        container.style.color = 'black';
      }
      document
        .querySelectorAll('#navbar-user-dp, #posting-dp')
    ...
    case /^[U-Zu-z]$/.test(name.charAt(0)):
      if (container) {
        container.style.background =
          'linear-gradient(135deg, #d1fbd0, #fbd1b7)';
        container.style.color = 'black';
      }
      document
        .querySelectorAll('#navbar-user-dp, #posting-dp')
