...
function countdown(minutes) {
  ...
  // setInterval  to run the code for every one second
  interval = setInterval(() => {
    ...
    if (distance <= 0) {
      ...
      otpMessage.textContent =
        "Didn't receive the code? click above to try again.";
    }
    if (counter) {
...
resendButton.addEventListener('click', async (event) => {
  ...
  const response = await fetch(`${API_URL}/api/user/resendOtp`, {
    method: 'POST',
    credentials: 'include',
    headers: {
      'Content-Type': 'application/json',
...
document.addEventListener('DOMContentLoaded', () => {
  form.addEventListener('submit', async (event) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/user/otpverification`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          ...
          clearInterval(interval);
          window.location.href = `${API_URL}/api/user/newPassword`;
        }, 1000);
      } else {
        successBox.innerHTML = '';
        successBox.style.display = 'none';
